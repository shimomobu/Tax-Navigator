...
async def lifespan(app: FastAPI):
    if os.getenv("TESTING") != "true":
        ...
        _services["llm"]           = OllamaLLM(base_url=OLLAMA_URL, model=OLLAMA_MODEL, temperature=0, model_kwargs={"options": {"temperature": 0}})
...
▼ 家事関連費の按分（所得税法施行令96条）
- 自宅家賃・電気代・通信費等で業務とプライベートが混在する場合
  → 業務使用の意思・実態が質問から読み取れる場合は「認められる」と断定すること
  → 按分割合の具体的な数値が不明でも「認められる」と判断してよい（割合は事後に決定できる）
  → 業務使用の実態が全く読み取れない場合のみ「グレーゾーン（要確認）」とする
- 按分根拠の記録が必要（面積比・時間比・通話明細等）だが、質問時点での未記録は否定理由にならない
...
▼ 以下のケースは必ず「経費として認められる」と断定すること（グレーゾーンにしてはならない）
- 取引先への贈答品（歳暮・中元・手土産等）→ 接待交際費として認められる
- 業務に関連する専門書籍・技術書・資格書籍の購入費 → 新聞図書費として認められる
- 青色事業専従者給与（届出済みの配偶者・家族への給与）→ 所得税法57条で認められる
- 自宅インターネット回線・スマートフォン（業務使用の実態がある場合）→ 通信費として認められる
- コワーキングスペースの利用料 → 地代家賃または会議費として認められる

▼ 以下のケースは必ず「経費として認められない」と断定すること
- 一人での食事代・ランチ代 → 家事費（所得税法45条）
- ビジネススーツ・私服（日常着用可能なもの）→ 家事費（所得税法45条）
- 個人の健康診断・医療費 → 家事費（所得税法45条）

▼ グレーゾーンとなるケース（本当に状況依存の場合のみ）
- 自宅から別の事務所への通勤費（自宅＝事業所でない場合）
- 現在の業務と直接関係しない新スキル習得の研修費
- 按分の根拠・割合が不明確な場合
- 業務関連性が質問文から全く読み取れない場合
...
def ask(
    ...
    # 2段階検索: 高信頼ソースを優先、裁決事例・通達は補完用
    from qdrant_client.models import Filter, FieldCondition, MatchAny

    primary_docs = vectorstore.similarity_search(
        expanded_query, k=8,
        filter=Filter(must=[FieldCondition(
            key="metadata.source",
            match=MatchAny(any=["nta_tax_answer", "nta_qa_cases"])
        )])
    )
    supplement_docs = vectorstore.similarity_search(
        expanded_query, k=7,
        filter=Filter(must=[FieldCondition(
            key="metadata.source",
            match=MatchAny(any=["tribunal_cases", "nta_tsutatsu"])
        )])
    )
    ...
    seen = set()
    candidates = []
    for doc in primary_docs + supplement_docs:
